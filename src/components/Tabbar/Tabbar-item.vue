<template>
  <li :class="{'tab-item': true, 'active': current == index }">
    <div class="item" @click="goTo">
      <div v-if="img" class="item-img-wrapper">
        <img class="item-img" :src="img" />
      </div>
      <div class="item-title">
        {{title}}
      </div>
    </div>
  </li>
</template>
<script>
import { mapGetters } from 'vuex';

export default {
  data() {
    return {
    }
  },
  props: {
    to: {
      type: String,
      default:"/shoppingCart"
    },
    title: {
      type: String,
      default: "shoppingCart"
    },
    img: {
      type: String,
      default: "data:image/png;base64,"
    },
    index: {
      type: Number,
      default: 0
    },
  },
  computed: {
    ...mapGetters(['current'])
  },
  create() {
    console.log(this.current);
  },
  methods: {
    goTo() {
      // console.log(this.to );
      this.$store.dispatch("ToggleIndex", this.index );
      this.$router.push({ path: this.to });
    }
  }
}
</script>

<style lang="scss" scoped>
$tabHeight: .88rem;
$imgRect: .44rem;
.tab-item {
  height: $tabHeight;
  flex: 1 0 auto;
  background-color: #fff;
  transition: all ease .3s;
  font-weight: bold;
  .item {
    color: #2c3e50;
    width: 100%;
    height: $tabHeight;
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
    line-height: 1;
    .item-img-wrapper {
      width: $imgRect;
      height: $imgRect;
      margin-bottom: 0.08rem;
      img {
        width: 100%;
        height: 100%;
      }
    }
    .item-title {
    }
  }
}
.tab-item.active {
  
  .item-title {
    color: #26a2ff;
  }
}
</style>